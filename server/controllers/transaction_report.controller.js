...
const transactionReport = async (req, res) => {
  try{
    ...
    let startDate = param?.start_date ? moment(param?.start_date).format('YYYY-MM-DD') : dateNow;
    let endDate = param?.end_date ? moment(param?.end_date).format('YYYY-MM-DD') : dateNow;
    ...
    // Start Query untuk menampilkan seluruh data
      ...
      for (let row of z) {
        ...
        if(row.transaksi_detail.length == 1){
          row.transaksi_detail = row.transaksi_detail.length == 0 ? {} : row.transaksi_detail[0];
        }
        rows.push(row);
      }
