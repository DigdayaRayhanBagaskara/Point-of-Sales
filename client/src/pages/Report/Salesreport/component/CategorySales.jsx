...
import { useGetSRCategorySalesQuery } from "../../../../redux/services/salesreportApi";
...
const ItemSales = () => {
  ...
  const categorySalesQuery = useGetSRCategorySalesQuery({
  ...
  const head = [
    {
      name: "Category Name",
      cell: (row) => <div className="w-64 break-words">{row.categories_name}</div>,
    },
    {
      name: "Jumlah Produk",
      cell: (row) => row.jumlah,
  ...
  const rows = categorySalesQuery?.data?.data?.rows.length > 0 ? categorySalesQuery?.data?.data?.rows : [{categories_name : "-", jumlah : "0", gross_sales : "0"}];


  // Ubah rows menjadi array dengan nama kolom di indeks pertama
  const transformedRows = [["Laporan Pertanggal : " + start_date +" - "+ end_date],["Category Name", "Jumlah", "Gross Sales"], ...rows.map(item => [item.categories_name, item.jumlah, item.gross_sales])];
  const createExcelFile = () => {
    ...
    XLSX.writeFile(wb, `Category Sales Report ${start_date} - ${end_date} _ ${moment().format("DDHHmmss")}.xlsx`);
  };
  return (
    <>
