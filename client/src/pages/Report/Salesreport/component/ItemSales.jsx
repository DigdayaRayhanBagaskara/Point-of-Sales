...
import { useGetSRItemSalesQuery } from "../../../../redux/services/salesreportApi";
...
const ItemSales = () => {
  ...
  const itemSalesQuery = useGetSRItemSalesQuery({
  ...
  const head = [
    {
      name: "Nama Produk",
      cell: (row) => <div className="w-64 break-words">{row.produk_name}</div>,
    },
    {
      name: "Jumlah Produk",
      cell: (row) => row.jumlah,
  ...
  const rows = itemSalesQuery?.data?.data?.rows.length > 0 ? itemSalesQuery?.data?.data?.rows : [{produk_name : "-", jumlah : "0", gross_sales : "0"}];


  // Ubah rows menjadi array dengan nama kolom di indeks pertama
  const transformedRows = [["Laporan Pertanggal : " + start_date +" - "+ end_date],["Produk Name", "Jumlah", "Gross Sales"], ...rows.map(item => [item.produk_name, item.jumlah, item.gross_sales])];
  const createExcelFile = () => {
    ...
    XLSX.writeFile(wb, `Item Sales Report ${start_date} - ${end_date} _ ${moment().format("DDHHmmss")}.xlsx`);
  };
  return (
    <>
