...
import { useGetSRBrandSalesQuery } from "../../../../redux/services/salesreportApi";
...
const ItemSales = () => {
  ...
  const brandSalesQuery = useGetSRBrandSalesQuery({
  ...
  const head = [
    {
      name: "Brand Name",
      cell: (row) => <div className="w-64 break-words">{row.brand_name}</div>,
    },
    {
      name: "Jumlah Produk",
      cell: (row) => row.jumlah,
  ...
  const rows = brandSalesQuery?.data?.data?.rows.length > 0 ? brandSalesQuery?.data?.data?.rows : [{brand_name : "-", jumlah : "0", gross_sales : "0"}];


  // Ubah rows menjadi array dengan nama kolom di indeks pertama
  const transformedRows = [["Laporan Pertanggal : " + start_date +" - "+ end_date],["Brand Name", "Jumlah", "Gross Sales"], ...rows.map(item => [item.brand_name, item.jumlah, item.gross_sales])];
  const createExcelFile = () => {
    ...
    XLSX.writeFile(wb, `Brand Sales Report ${start_date} - ${end_date} _ ${moment().format("DDHHmmss")}.xlsx`);
  };
  return (
    <>
