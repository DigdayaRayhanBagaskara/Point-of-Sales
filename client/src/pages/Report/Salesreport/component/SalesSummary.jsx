...
import { useGetSRSalesSummaryQuery } from "../../../../redux/services/salesreportApi";
...
const SalesSummary = () => {
  ...
  const salesSummaryQuery = useGetSRSalesSummaryQuery({
  ...
  // Ubah rows menjadi array dengan nama kolom di indeks pertama
    const transformedRows = [["Laporan Pertanggal : " + start_date +" - "+ end_date],[rows[0].name,rows[0].isi],[rows[1].name,rows[1].isi],[rows[2].name,rows[2].isi]];
    const createExcelFile = () => {
      ...
      XLSX.writeFile(wb, `Sales Summary Report ${start_date} - ${end_date} _ ${moment().format("DDHHmmss")}.xlsx`);
    };
