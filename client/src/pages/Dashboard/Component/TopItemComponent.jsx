...
import { useGetTopItemDashboardQuery } from "../../../redux/services/dashboardApi";
import Table from "./TableComponent";

const TopItemComponent = ({ params }) => {
  const topItemQuery = useGetTopItemDashboardQuery({
  ...
  const rows = topItemQuery?.data?.data.length ? topItemQuery?.data?.data : [{produk_name : "-", jumlah : "0"}]
  return (
    <>
      <Card className="border border-gray-500 rounded p-5">
        <div className="mb-2">
          <div className="text-gray-700  text-md mb-2">TOP ITEMS</div>
